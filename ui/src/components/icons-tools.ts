...
export const toolIcons = {
  ...
  pinOff: strokeIcon(svg` <line x1="2" x2="22" y1="2" y2="22" />
    ...
    />`),
  // Session pin state, drawn as a pair so the silhouette alone reports it: the
  // hollow body means "not pinned", the solid body means "pinned". The solid
  // one must stay in the neutral control color — unpinning is not destructive,
  // so a danger or accent tint would overstate it.
  pinHollow: fillIcon(svg`<path
    clip-rule="evenodd"
    fill-rule="evenodd"
